...
// 2. 응답 데이터 객체 출력
async function getASingleCarts() {
  const response = await axios.get(`${BASE_URL}/carts/10`);
  const data = response["data"];
  console.log(data);
}
//getASingleCarts();
...
async function getNewProducts() {
  ...
  const values = products.map((products) => products["title"]);
  const values2 = products.map((products) => {
