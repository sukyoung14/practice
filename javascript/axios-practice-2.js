...
async function getProduct() {
  const config = { url: `${DOMAIN_NAME}/products/10`, method: "get" };
...
const getProductReviewsRating = async () => {
  const config = { url: `${DOMAIN_NAME}/products/10`, method: "get" };
  const response = await axios(config);
  const data = response["data"];
  const reviews = data["reviews"];
  let sum = 0;
  const values = reviews.map((reviews) => reviews.rating);
...
const getPosts = async () => {
  const config = { url: `${DOMAIN_NAME}/posts`, method: "get" };
  const response = await axios(config);
  const data = response["data"];
  const posts = data["posts"];
  const values = posts.map((posts) => posts.tags);
...
const getTagPosts = async () => {
  const config = { url: `${DOMAIN_NAME}/posts/tag/life`, method: "get" };
  const response = await axios(config);
  const data = response["data"];
...
const getbooks = async () => {
  const config = {
    url: `${DOMAIN_NAME}/posts/search?q=book`,
    method: "get",
  };
  const response = await axios(config);
  const data = response["data"];
...
const getProducts = async (search) => {
  const config = {
    url: `${DOMAIN_NAME}/products/search?q=${search}`,
    method: "get",
  };
  const response = await axios(config);
  const data = response["data"];

  console.log(`${search}검색 시 : ${data["total"]}`);
};
getProducts("glass");
getProducts("phone");
/* 출력 결과
